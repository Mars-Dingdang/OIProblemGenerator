// Library Checker - K-Shortest Walk
#include <iostream>
#include <queue>
#include <vector>
#include <random>
...
struct PersistentRandomizedHeap {
  ...
  int meld(int x, int y, uint64_t rand) {
    if (!x || !y) return x | y;
  ...
  void insert(int i, long long cost, int _to) {
    rt[i] = meld(rt[i], new_node(cost, _to), std::mt19937_64(time(nullptr))());
  }

  void merge(int i, int j) {
    rt[i] = meld(rt[i], rt[j], std::mt19937_64(time(nullptr))());
  }
};
...
void build_sidetracks(PersistentRandomizedHeap& heaps) {
  for (int i = 0; i < m; ++i) {
    auto edge = edges[i];
...
void insert(std::priority_queue<std::pair<long long, int>,
                         std::vector<std::pair<long long, int>>, std::greater<>>& pq,
            const PersistentRandomizedHeap& heaps, int x, long long cost) {
  if (x) pq.emplace(cost + heaps.va[x], x);
}

void find_k_shortest_walks(PersistentRandomizedHeap& heaps) {
  int cnt = 0;
  ans.assign(k, -1);
  if (dist_t[s] >= inf) return;
  ans[cnt++] = dist_t[s];
  insert(pq, heaps, heaps.rt[s], dist_t[s]);
  ...
  while (!pq.empty() && cnt < k) {
    ...
    pq.pop();
    ans[cnt++] = cost;
    insert(pq, heaps, heaps.lc[cur], cost - heaps.va[cur]);
    insert(pq, heaps, heaps.rc[cur], cost - heaps.va[cur]);
    insert(pq, heaps, heaps.rt[heaps.to[cur]], cost);
  }
}

int main() {
  std::cin >> n >> m >> s >> t >> k;
  gr.resize(n); ig.resize(n);
  edges.reserve(m);
  for (int i = 0; i < m; ++i) {
    int u, v, c;
    std::cin >> u >> v >> c;
  ...
  calc_distances_to_t();
  PersistentRandomizedHeap heaps;
  build_sidetracks(heaps);
  find_k_shortest_walks(heaps);
