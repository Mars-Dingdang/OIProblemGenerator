...
%    - Initialize the subsequence automaton state to 0.
%    - For each ending index $j$ from $i$ to $n$:
%         * Update the subsequence automaton state with character $A[j]$. Let the new state be $p$.
%         * If $p > n$, break (no further extensions will be subsequences).
        * Let $X =